...
public class XYPlotSp extends JFrame {
    ...
    private ActionsC act;
    
    public XYPlotSp(Spectra spectraDrew, final String titleWindow,final String titleGraph2, double[] energiesX, double[] dataY,int nROI) {
        super(titleWindow);
        this.parentWindow=spectraDrew.getParentWindowS();
        this.titleGraph=titleGraph2;
        this.drawSpectra=spectraDrew;
        //create the dataset
        final XYDataset dataset = createDataset(energiesX,dataY);
        //create the chart
        final JFreeChart myChart = createChart(dataset,titleGraph2,checkCalib());
        this.chart = myChart;
    ...
    private void initComponents(int nROI) {
        ...
        for (int i=0; i<nbLines;i++){
            ...
            IJ.log("IJ Pref / nROI = "+String.valueOf(nROI));
    ...
    private void jButtonGenImgActionPerformed(java.awt.event.ActionEvent evt) {                                         
        ...
        /*try {
            ...
            if(minMaxSpectraArray.length>0){
                tabImgGenFromSpectra = drawSpectra.elementMaps(minMaxSpectraArray);
        ...
        act.generateMaps(drawSpectra, getValuesMinMaxNames(false,true,true));
    }
                    ...
                    if(minOrMax|| !((min.getText().equals(tr("Min")) || max.getText().equals(tr("Max"))))){
                        if ( !nameIsImportant || !(name.getText().equals(tr("Name"))) ) {
                            ...
                            if(minOrMax||start<end){
                                if (drawSpectra.isAvailable(start) || drawSpectra.isAvailable(end)){
                                    if (!drawSpectra.isAvailable(start)){
                                        start=drawSpectra.getEnergies()[0];
                                    }
                                    else if(!drawSpectra.isAvailable(end)) {
                                        int length = drawSpectra.getEnergies().length;
                                        end=drawSpectra.getEnergies()[length-1];
                                    }
    ...
    public float getRealXValue(double value, boolean rightIncluded){
        if(drawSpectra.isAvailable((float) value)){
            int indiceNearValue=drawSpectra.getIndex((float)value,rightIncluded);
            return drawSpectra.getEnergies()[indiceNearValue];
        }
        return -1;
    }

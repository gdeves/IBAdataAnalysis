...
public class ADC{
 ...
 /*   public void readSupaVisioTypeEventList(DataInputStream ips){
    ...
    private int[] medianIntTable(){
        int [] values=new int[median.size()];
        for (int x=0;x<sizeMapX;x++) {
            for (int y=0;y<sizeMapY;y++){
                    values[(x)+sizeMapX*(y)]= median.get(x+sizeMapX*y);
            }
        }
        return values;
    }
        public void saveMedianImage(String path){
            ...
            ImageProcessor ip = new FloatProcessor(sizeMapX-2,sizeMapY-2,medianIntTable()); 
            ImagePlus imp = new ImagePlus("stim",ip); 
